	...
	<div class="news-in">
		...
		<ul class="news">
			<li id="<portlet:namespace />news1" class="here">
				<a title="<%= newsMap.get("topNewsText") %>" href="<%= newsMap.get("topNewsURL") %>">
					<%= newsMap.get("topNewsText") %><span><%= format.format(((Calendar) newsMap.get("topNewsDate")).getTime()) %></span></a>
				<i></i>
			</li>
			<li id="<portlet:namespace />news2">
				<a title="<%= newsMap.get("middleNewsText") %>" href="<%= newsMap.get("middleNewsURL") %>"><%= newsMap.get("middleNewsText") %><span><%= format.format(((Calendar) newsMap.get("middleNewsDate")).getTime()) %></span></a>
				<i></i>
			</li>
			<li id="<portlet:namespace />news3" >
				<a title="<%= newsMap.get("bottomNewsText") %>" href="<%= newsMap.get("bottomNewsURL") %>"><%= newsMap.get("bottomNewsText") %><span><%= format.format(((Calendar) newsMap.get("bottomNewsDate")).getTime()) %></span></a>
				<i></i>
			</li>
		</ul>
	</div>
